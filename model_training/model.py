...
class LSTMTCNRegressor(nn.Module):
    def __init__(self,
        ...
        self.crop_embedding = nn.Embedding(num_crops, fips_embedding_dim)  

        lstm_in = input_dim + fips_embedding_dim  # embedding gets added after summing crop+fips
    ...
    def forward(self, x_weather, fips_ids, crop_ids):
        emb_fips = self.fips_embedding(fips_ids)          # (B, E)
        emb_crop = self.crop_embedding(crop_ids)          # (B, E)
        emb = emb_fips + emb_crop                         # (B, E)
        emb = emb.unsqueeze(1).repeat(1, x_weather.size(1), 1)
        x = torch.cat([x_weather, emb], dim=-1)
        out, _ = self.lstm(x)
