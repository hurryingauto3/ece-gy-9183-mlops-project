...
class OpenStackSettings(BaseSettings):
    ...
    auth_type: str = Field("password", description="OpenStack authentication type (e.g., 'password', 'application_credential').")
    ...
    def check_auth_fields(self) -> 'OpenStackSettings':
        if self.auth_type == 'application_credential':
            if not self.application_credential_id:
                raise ValueError('OS_APPLICATION_CREDENTIAL_ID must be set for auth_type "application_credential"')
            if not self.application_credential_secret:
                 raise ValueError('OS_APPLICATION_CREDENTIAL_SECRET must be set for auth_type "application_credential"')
        ...
        elif self.auth_type not in ['token', 'v2password', 'v3password', 'v3token', 'v3oidcpassword', 'application_credential']:
