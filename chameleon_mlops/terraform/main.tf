...
data "openstack_networking_network_v2" "external" {
  ...
  name        = var.ext_net_name    # e.g., "public"
}
...
resource "openstack_networking_subnet_v2" "private_subnet" {
  ...
  ip_version       = 4
  gateway_ip       = var.network_gateway  # e.g., "192.168.100.1"
  allocation_pools {
    start = var.network_pool_start     # e.g., "192.168.100.50"
    end   = var.network_pool_end       # e.g., "192.168.100.200"
  }
  dns_nameservers = var.dns_nameservers  # e.g., ["8.8.8.8","8.8.4.4"]
}
...
resource "openstack_networking_secgroup_v2" "secgrp" {
  ...
  description = "Ingress rules for MLOps"
}
...
resource "openstack_compute_instance_v2" "services_node" {
  ...
  security_groups = [openstack_networking_secgroup_v2.secgrp.name]
  networks        = [{ uuid = openstack_networking_network_v2.private_net.id }]
}
...
# Optional staging node on CHI@TACC
...
resource "openstack_compute_floatingip_associate_v2" "assoc_staging" {
  count       = var.enable_staging ? 1 : 0
  provider    = openstack.chi
  floating_ip = openstack_networking_floatingip_v2.fip_staging.*.address
  instance_id = openstack_compute_instance_v2.staging_node.*.id
}
